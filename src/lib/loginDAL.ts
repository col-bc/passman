...
export async function recordLogin(loginData: Omit<Login, 'id' | 'createdAt'>): Promise<DALResult<Login>> {
  try {
...
export async function getLoginsByUserId(userId: string): Promise<DALResult<Login[]>> {
  try {
