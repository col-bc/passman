...
export async function getItemsByLockerId(lockerId: string): Promise<DALResult<Item[]>> {
  try {
...
export async function getItemById(itemId: string): Promise<DALResult<Item | null>> {
  try {
...
export async function updateItem(
  ...
  try {
    ...
    if (updatedFields.encryptedData) {
      ...
      updateData.ciphertext = Buffer.from(Buffer.from(ciphertext).toString('base64'));
